...
public class LeafNode extends QuadTreeNode {
    ...
    public QuadTreeNode insert(Point point, int x, int y, int size) {
        ...
        if (this.points.isEmpty()) {
            ...
            // Clear the points in the current leaf (now unnecessary since they're moved to the internal node).
            this.points.clear();
    ...
    public List<Point> regionSearch(int queryX, int queryY, int width, int height, int nodeX, int nodeY, int nodeSize) {
        return points.stream()
                     .filter(p -> p.getX() >= queryX && p.getX() <= queryX + width &&
                                  p.getY() >= queryY && p.getY() <= queryY + height)
                     .collect(Collectors.toList());
    }
