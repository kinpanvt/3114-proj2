 ...
 * by sharing a single instance to represent all empty leaf nodes in the PR Quadtree.
 */
public class FlyweightNode extends QuadTreeNode {
    ...
    /**
     * Inserts a point into the PR Quadtree. This method is overridden in the FlyweightNode
     * to perform no action directly, as the FlyweightNode represents an empty quadrant.
    ...
    public QuadTreeNode insert(Point point, int x, int y, int size) {
        ...
        return newNode.insert(point, x, y, size); // Delegate to the new leaf node's insert method.
    }

    /**
     * Removes a point from the PR Quadtree. This method is overridden in the FlyweightNode
     * to always return false, as the FlyweightNode represents an empty quadrant where
     ...
     * This method is overridden in the FlyweightNode to always return an empty list,
     * as the FlyweightNode represents an empty quadrant where no points are stored.
     *
    ...
    /**
     * Performs a region search in the PR Quadtree. This method is overridden in the FlyweightNode
     * to always return an empty list, as the FlyweightNode represents an empty quadrant where
     ...
     * @param nodeX The x-coordinate of the top-left corner of the current quadrant.
     * @param nodeY The y-coordinate of the top-left corner of the current quadrant.
     * @param nodeSize The size of the current quadrant.
     * @return An empty list, as no points exist in the FlyweightNode.
     */
    @Override
    public List<Point> regionSearch(int queryX, int queryY, int width, int height, int nodeX, int nodeY, int nodeSize) {
        return Collections.emptyList(); // No points in FlyweightNode for any region.
    }
}
